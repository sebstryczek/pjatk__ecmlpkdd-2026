...
class SemSegTester(TesterBase):
    def test(self):
        ...
        logger.info("updated >>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
...
class DINOSemSegTester(TesterBase):
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        logger.info("1111>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
...
class ClsTester(TesterBase):
    def test(self):
        logger = get_root_logger()
        logger.info("22222>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
        batch_time = AverageMeter()
        intersection_meter = AverageMeter()
        union_meter = AverageMeter()
        target_meter = AverageMeter()
...
class ClsVotingTester(TesterBase):
    ...
    def test(self):
        for i in range(self.num_repeat):
            logger = get_root_logger()
            logger.info(f"3333>>>>>>>>>>>>>>>> Start Evaluation {i + 1} >>>>>>>>>>>>>>>>")
...
class PartSegTester(TesterBase):
    def test(self):
        ...
        logger = get_root_logger()
        logger.info("44444>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
...
class InsSegTester(TesterBase):
    ...
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        logger.info("55555>>>>>>>>>>>>>>>> Start Evaluation >>>>>>>>>>>>>>>>")
